...
{
    public partial class FrmReportTratamientos : MaterialSkin.Controls.MaterialForm
    {
        public FrmReportTratamientos()
        {
            InitializeComponent();
        }

        private void FrmReportTratamientos_Load(object sender, EventArgs e)
        {
        ...
        private void btnPDF_Click(object sender, EventArgs e)
        {
            ...
            {    //ESCOJE A RUTA DONDE GUARDAREMOS EL PDF 
                ...
                if (save.ShowDialog() == DialogResult.OK)
                {
                    ...
                    Chunk encab1 = new Chunk("DIRECCION :Asoc.Villa la Paz Mz.A Lt 11        TELEFONO :+51 990270475                     ", FontFactory.GetFont("COURIER", 18)); ;
                    Chunk encab2 = new Chunk("CLIENTE : " + FrmHistorial.Rnombres, FontFactory.GetFont("COURIER", 18));
                    Chunk encab3 = new Chunk("TIPO DE INFORMACION :Reportes de Tratamientos de Mascota", FontFactory.GetFont("COURIER", 18));
        ...
        {
            PdfPTable datatable = new PdfPTable(dgvTratamientos.ColumnCount);

            datatable.DefaultCell.Padding = 1;
            float[] headerwidths = GetTamañoColumnas(dgvTratamientos);
            ...
            for (int i = 0; i < dgvTratamientos.ColumnCount; i++)
            {

                objP = new Phrase(dgvTratamientos.Columns[i].HeaderText, fuente);
            ...
            datatable.DefaultCell.BorderWidth = 1;
            for (int i = 0; i < dgvTratamientos.RowCount; i++)
            {
                for (int j = 0; j < dgvTratamientos.ColumnCount; j++)
                {
                    objP = new Phrase(dgvTratamientos[j, i].Value.ToString(), fuente);
                    datatable.AddCell(objP);
                }
