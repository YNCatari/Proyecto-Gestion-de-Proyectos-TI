...
{
    public partial class FrmReportVacunas : MaterialSkin.Controls.MaterialForm
    {
        public FrmReportVacunas()
        {
            InitializeComponent();
        }

        private void FrmReportVacunas_Load(object sender, EventArgs e)
        {
            ...
            dgvVacunas.DataSource = objN.MtdBuscarVacunas(objE);
            dgvVacunas.AutoResizeColumns();
        }
        ...
        private void btnPDF_Click(object sender, EventArgs e)
        {
            ...
            {    //ESCOJE A RUTA DONDE GUARDAREMOS EL PDF 
                ...
                if (save.ShowDialog() == DialogResult.OK)
                {
                    ...
                    Chunk encab0 = new Chunk(" Happy Pets Veterinaria & Spa", FontFactory.GetFont("Arial Black", 25)); ;
                    Chunk encab1 = new Chunk("DIRECCION :Asoc.Villa la Paz Mz.A Lt 11        TELEFONO :+51 990270475s                     ", FontFactory.GetFont("COURIER", 18)); ;
                    Chunk encab2 = new Chunk("CLIENTE : " + FrmHistorial.Rnombres, FontFactory.GetFont("COURIER", 18));
                    Chunk encab3 = new Chunk("TIPO DE INFORMACION : Reporte de Vacunas de Mascota", FontFactory.GetFont("COURIER", 18));
        ...
        {
            PdfPTable datatable = new PdfPTable(dgvVacunas.ColumnCount);

            datatable.DefaultCell.Padding = 1;
            float[] headerwidths = GetTamañoColumnas(dgvVacunas);
            ...
            for (int i = 0; i < dgvVacunas.ColumnCount; i++)
            {

                objP = new Phrase(dgvVacunas.Columns[i].HeaderText, fuente);
            ...
            datatable.DefaultCell.BorderWidth = 1;
            for (int i = 0; i < dgvVacunas.RowCount; i++)
            {
                for (int j = 0; j < dgvVacunas.ColumnCount; j++)
                {
                    objP = new Phrase(dgvVacunas[j, i].Value.ToString(), fuente);
                    datatable.AddCell(objP);
                }
